...
const fixedApertureGrid: Factory = () => ({
  draw(ctx, w, h, t, p, palette) {
    ctx.clearRect(0, 0, w, h);
    const angleRad = (p.sunAngle / 180) * Math.PI;
    // Scaling the offset by cos/sin alone would keep the offset VECTOR's
    // length ~constant (cos²+sin²=1), so the shadow only spun in place as
    // sunAngle changed and never visibly lengthened or shortened — the
    // opposite of real shadows (low sun = long, high sun = short).
    // elevationFactor scales the offset AND stretches the shadow ellipse
    // itself so length is legible, not just direction. The aperture radius
    // (r) is deliberately untouched: apertures staying a constant size is
